...
{
	rsize_t rown, column;
	std::cout << "input rown "; std::cin >> rown;
	std::cout << " inoyut column"; std::cin >> column;
	std::vector<std::vector<int>>x(70, std::vector<int>(76));


	for (rsize_t i = 0; i < 70; ++i)
	{
		for (rsize_t j = 0; j < 76; ++j)
		{
			x[i][j] = 0;
		}
	}
	...
	char c;
	std::cout << "now to input operator throught character  "; std::cin >> c;
	...
	int number1, number2;
	std::cout << "\n\ninput first number  for operation"; std::cin >> number1;
	std::cout << "\n\ninput second number for operatiion"; std::cin >> number2;
	std::cout << u_ptr->operator()(number1, number2) << std::endl;
	std::cout << "\n\n\n\n";

	rsize_t n_row, n_column, old_row, old_column;
	int const_value;
std::	cout << "\n\ninput i and j index,where we want to add sum "; std::cin >> n_row >> n_column;

std::cout << "\n\ninput index of element,form where we have to use ";;std:: cin >> old_row >> old_column;
std::cout << "\n\ninput const value ";std:: cin >> const_value;
