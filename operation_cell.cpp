...
int operation_cell::operator()(const array_2D<abstract_cell_ptr> &data) const
{
	...
	for (int i = 0; i < m_operator.size();)
	{
		...
		if (m_operator[i] == 'c')
		{
			...
			auto& operation_cell = (*data[cur_indexes.first][cur_indexes.second]);
			if (!operation_cell) {
				throw "Theer is a reference to an empty cell!";
			}

			// If there was an operator before the operation, i.e. ... + c(i j)
			if (op_sign != '?')
			{
				res = util::get_fn(op_sign)(res, operation_cell(data));
				op_sign = '?';
			}
			else
			{
				// If it's the first operand, e.g. c(10 12)
				res = operation_cell(data);
			}
		}
		else
		{
